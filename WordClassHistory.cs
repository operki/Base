...
namespace WindowsFormsApp1
{
    public class GeneratedClassHistory
    {
        ...
        private string PrimaryDecline(string primaryName)
        {
            switch (primaryName)
            {
                case "рядовой":
        ...
        public void CreatePackage(string filePath, string sqlConnectionString, int peopleId, bool[] nok, int nokId)
        {
            _nok = nok;
            ...
            var fioNames = new Decliner().Decline(_sqlReader["fio0"].ToString(), _sqlReader["fio1"].ToString(),
                _sqlReader["fio2"].ToString(), 2);
            _fio = fioNames[0] + " " + fioNames[1] + " " + fioNames[2];
            var userPositionName = _sqlReader["positionId"].ToString();
            var userPositionDate = _sqlReader["positionOrderId"].ToString();
            var userBirthday = Convert.ToDateTime(_sqlReader["dateBirthday"]);
            var tempId = _sqlReader["primaryId"];
            _sqlReader?.Close();
            _sqlCommand = new SqlCommand("SELECT [fullName] FROM [Positions] WHERE [id]=@id", _sqlConnection);
            _sqlCommand.Parameters.AddWithValue("id", Convert.ToInt32(userPositionName));
            _sqlReader = _sqlCommand.ExecuteReader();
            _sqlReader.Read();
            //полное наименование текущей должности
            userPositionName = _sqlReader["fullName"].ToString();
            _sqlReader?.Close();
            ...
            _sqlReader = _sqlCommand.ExecuteReader();
            _sqlReader.Read();
            //склонение звания в родительный
            _fio = PrimaryDecline(_sqlReader["name"].ToString()) + " " + _fio;
            _sqlReader?.Close();
            ...
            //выгрузка послужного списка
            _sqlCommand = new SqlCommand("SELECT [name], [orderId] FROM [History] WHERE [peopleId]=@peopleId", _sqlConnection);
            _sqlCommand.Parameters.AddWithValue("peopleId", peopleId);
            _sqlReader = _sqlCommand.ExecuteReader();
            var j = 0;
            DateTime[] historyDates =
            {
                DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue,
                DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue,
                DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue,
                DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue,
                DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue, DateTime.MaxValue
            };
            string[] historyDateStrings =
            {
                "", "", "", "", "", "", "", "", "", "",
                "", "", "", "", "", "", "", "", "", "",
                "", "", "", "", "", "", "", "", "", ""
            ...
            //заполнение послужного списка
            if (_sqlReader.HasRows)
                while (_sqlReader.Read())
                {
                    _historyStrings[j] = _sqlReader["name"].ToString();
                    historyDateStrings[j] = _sqlReader["orderId"].ToString();
                    j++;
                }
            _sqlReader.Close();
            _historyStrings[j] = userPositionName + ", войсковая часть" + (Char)160 + "71289 (г." + (Char)160 + "Уссурийск Приморского края)";
            historyDateStrings[j] = userPositionDate;

            //вытаскиваем даты из приказов
            for (var i = 0; i < historyDateStrings.Length; i++)
            {
                if (historyDateStrings[i] != "")
                {
                    _sqlCommand = new SqlCommand("SELECT [date] FROM [Orders] WHERE [id]=@id", _sqlConnection);
                    _sqlCommand.Parameters.AddWithValue("id", Convert.ToInt32(historyDateStrings[i]));
                    _sqlReader = _sqlCommand.ExecuteReader();
                    _sqlReader.Read();
                    historyDates[i] = Convert.ToDateTime(_sqlReader["date"]);
                    _sqlReader.Close();
                }
            }
            
            //сортировка дат по возрастанию вместе с текстом
            for (int i = 0; i < historyDates.Length - 1; i++)
                for (int i2 = i + 1; i2 < historyDates.Length; i2++)
                    if (historyDates[i] > historyDates[i2])
                    {
                        var temp = historyDates[i];
                        historyDates[i] = historyDates[i2];
                        historyDates[i2] = temp;
                        var temp2 = _historyStrings[i];
                        _historyStrings[i] = _historyStrings[i2];
                        _historyStrings[i2] = temp2;
                    }
            
            //скрепляем дату с текстом
            var findEnd = false;
            for (var i = 0; i < historyDateStrings.Length; i++)
            {
                if (!findEnd && historyDates[i] != DateTime.MaxValue)
                {
                    var nextExist = (i < historyDateStrings.Length - 1
                                      && historyDates[i + 1] != DateTime.MaxValue);
                    historyDateStrings[i] = "c " + historyDates[i].ToString("dd.MM.yyyy") + (Char)160 + "г. по ";
                    historyDateStrings[i] += nextExist
                        ? historyDates[i + 1].ToString("dd.MM.yyyy") + (Char)160 + "г."
                        : "настоящее время";
                    _historyStrings[i] = historyDateStrings[i] + " – " + _historyStrings[i];
                    _historyStrings[i] += nextExist ? ";" : ".";
                }
                else
                    findEnd = true;
            }
            ...
            //выгрузка контракта
            _sqlCommand = new SqlCommand("SELECT [slaveStart], [slaveEnd] FROM [Slaves] WHERE [peopleId]=@peopleId", _sqlConnection);
            _sqlCommand.Parameters.AddWithValue("peopleId", peopleId);
            _sqlReader = _sqlCommand.ExecuteReader();
            _sqlReader.Read();
            var dateStart = Convert.ToDateTime(_sqlReader["slaveStart"]);
            var dateEnd = Convert.ToDateTime(_sqlReader["slaveEnd"]);
            _sqlReader.Close();
            _slaveString = "с " + dateStart.ToString("dd.MM.yyyy") + (Char)160 + "г.";
            var days = (dateEnd - dateStart).TotalDays;
            var daysLong = "";
            if (days > 363 && days < 367)
                daysLong = " на 1" + (Char)160 + "(один) год ";
            else if (days > 727 && days < 733)
                daysLong = " на 2" + (Char)160 + "(два) года ";
            else if (days > 1091 && days < 1099)
                daysLong = " на 3" + (Char)160 + "(три) года ";
            else if (days > 1820 && days < 1830)
                daysLong = " на 5" + (Char)160 + "(пять) лет ";
            else if (days > 3644 && days < 3656)
                daysLong = " на 10" + (Char)160 + "(десять)) лет ";
            else
            {
                days = Convert.ToInt32(((dateEnd - userBirthday).Days / 365 -
                       Convert.ToInt32((dateEnd - userBirthday).Days / 365)) * 100);
                if (dateEnd.Day == userBirthday.Day && dateEnd.Month == userBirthday.Month)
                    daysLong = " до наступления предельного возраста пребывания на военной службе ";
                else
                    daysLong = " на время обучения и 5" + (Char)160 + "(пять) лет после окончания ";
            }
            _slaveString += daysLong + "по " + dateEnd.ToString("dd.MM.yyyy") + (Char)160 + "г.";
        ...
        private void GenerateExtendedFilePropertiesPart1Content(ExtendedFilePropertiesPart extendedFilePropertiesPart1)
        {
            ...
            charactersWithSpaces1.Text = "1047";
        ...
        private void GenerateMainDocumentPart1Content(MainDocumentPart mainDocumentPart1)
        {
            ...
            Paragraph paragraph1 = new Paragraph() { RsidParagraphMarkRevision = "00717B75", RsidParagraphAddition = "00A65B84", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00A65B84" };
            ...
            Paragraph paragraph2 = new Paragraph() { RsidParagraphMarkRevision = "00717B75", RsidParagraphAddition = "00A65B84", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00A65B84" };
            ...
            Paragraph paragraph3 = new Paragraph() { RsidParagraphMarkRevision = "00717B75", RsidParagraphAddition = "00A65B84", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00A65B84" };
            ...
            Paragraph paragraph4 = new Paragraph() { RsidParagraphAddition = "00A65B84", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00A65B84" };
            ...
            Paragraph paragraph5 = new Paragraph() { RsidParagraphMarkRevision = "00D05B95", RsidParagraphAddition = "00A65B84", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00A65B84" };
            ...
            Paragraph paragraph6 = new Paragraph() { RsidParagraphMarkRevision = "00D05B95", RsidParagraphAddition = "00A65B84", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00A65B84" };
            ...
            Paragraph paragraph7 = new Paragraph() { RsidParagraphMarkRevision = "00D05B95", RsidParagraphAddition = "00A65B84", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00A65B84" };
            ...
            Paragraph paragraph8 = new Paragraph() { RsidParagraphMarkRevision = "00D05B95", RsidParagraphAddition = "00A65B84", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00D55764" };
            ...
            runProperties1.Append(bold6);
            runProperties1.Append(noProof1);
            runProperties1.Append(fontSize9);
            ...
            V.Shape shape1 = new V.Shape() { Id = "_x0000_s1026", Style = "position:absolute;left:0;text-align:left;margin-left:-.05pt;margin-top:-99.85pt;width:148.65pt;height:199.05pt;z-index:251658240", Filled = false, Stroked = false, Type = "#_x0000_t202" };

            V.TextBox textBox1 = new V.TextBox();

            TextBoxContent textBoxContent1 = new TextBoxContent();

            Paragraph paragraph9 = new Paragraph() { RsidParagraphAddition = "00444948", RsidParagraphProperties = "00444948", RsidRunAdditionDefault = "00444948" };
            ...
            paragraphMarkRunProperties9.Append(runFonts1);
            paragraphMarkRunProperties9.Append(bold7);
            paragraphMarkRunProperties9.Append(fontSize10);
            paragraphMarkRunProperties9.Append(fontSizeComplexScript1);

            paragraphProperties9.Append(paragraphBorders1);
            ...
            Wp.Inline inline1 = new Wp.Inline() { DistanceFromTop = (UInt32Value)0U, DistanceFromBottom = (UInt32Value)0U, DistanceFromLeft = (UInt32Value)0U, DistanceFromRight = (UInt32Value)0U, AnchorId = "3DBC9D42", EditId = "07E9C851" };
            ...
            paragraph9.Append(run2);
            paragraph9.Append(run3);

            Paragraph paragraph10 = new Paragraph() { RsidParagraphMarkRevision = "0034690F", RsidParagraphAddition = "00444948", RsidParagraphProperties = "00444948", RsidRunAdditionDefault = "00444948" };
            ...
            paragraphMarkRunProperties10.Append(fontSizeComplexScript3);

            paragraphProperties10.Append(paragraphBorders2);
            ...
            paragraph10.Append(proofError1);
            paragraph10.Append(run4);
            paragraph10.Append(proofError2);

            Paragraph paragraph11 = new Paragraph() { RsidParagraphMarkRevision = "0034690F", RsidParagraphAddition = "00444948", RsidParagraphProperties = "00444948", RsidRunAdditionDefault = "00444948" };
            ...
            paragraphMarkRunProperties11.Append(runFonts5);
            paragraphMarkRunProperties11.Append(fontSizeComplexScript5);

            paragraphProperties11.Append(paragraphBorders3);
            ...
            paragraph11.Append(proofError3);
            paragraph11.Append(run5);
            paragraph11.Append(proofError4);

            Paragraph paragraph12 = new Paragraph() { RsidParagraphMarkRevision = "0034690F", RsidParagraphAddition = "00444948", RsidParagraphProperties = "00444948", RsidRunAdditionDefault = "00444948" };
            ...
            paragraphMarkRunProperties12.Append(runFonts7);
            paragraphMarkRunProperties12.Append(bold10);
            paragraphMarkRunProperties12.Append(fontSizeComplexScript7);

            paragraphProperties12.Append(paragraphBorders4);
            ...
            Paragraph paragraph13 = new Paragraph() { RsidParagraphMarkRevision = "0034690F", RsidParagraphAddition = "00444948", RsidParagraphProperties = "00444948", RsidRunAdditionDefault = "00444948" };
            ...
            paragraphMarkRunProperties13.Append(runFonts10);
            paragraphMarkRunProperties13.Append(bold12);
            paragraphMarkRunProperties13.Append(fontSize12);
            paragraphMarkRunProperties13.Append(fontSizeComplexScript10);

            paragraphProperties13.Append(paragraphBorders5);
            ...
            Paragraph paragraph14 = new Paragraph() { RsidParagraphMarkRevision = "0034690F", RsidParagraphAddition = "00444948", RsidParagraphProperties = "00444948", RsidRunAdditionDefault = "00444948" };
            ...
            paragraphMarkRunProperties14.Append(runFonts11);
            paragraphMarkRunProperties14.Append(bold13);
            paragraphMarkRunProperties14.Append(fontSize13);
            paragraphMarkRunProperties14.Append(fontSizeComplexScript11);

            paragraphProperties14.Append(paragraphBorders6);
            paragraphProperties14.Append(indentation8);
            ...
            Paragraph paragraph15 = new Paragraph() { RsidParagraphMarkRevision = "0034690F", RsidParagraphAddition = "00444948", RsidParagraphProperties = "00444948", RsidRunAdditionDefault = "00444948" };
            ...
            paragraphMarkRunProperties15.Append(runFonts14);
            paragraphMarkRunProperties15.Append(fontSize16);
            paragraphMarkRunProperties15.Append(fontSizeComplexScript14);

            paragraphProperties15.Append(paragraphBorders7);
            paragraphProperties15.Append(indentation9);
            ...
            Paragraph paragraph16 = new Paragraph() { RsidParagraphMarkRevision = "0034690F", RsidParagraphAddition = "00444948", RsidParagraphProperties = "00444948", RsidRunAdditionDefault = "00444948" };
            ...
            Indentation indentation10 = new Indentation() { Start = "-142", End = "-150" };
            ...
            paragraphMarkRunProperties16.Append(fontSize18);
            paragraphMarkRunProperties16.Append(fontSizeComplexScript16);

            paragraphProperties16.Append(paragraphBorders8);
            paragraphProperties16.Append(indentation10);
            ...
            Paragraph paragraph17 = new Paragraph() { RsidParagraphMarkRevision = "0034690F", RsidParagraphAddition = "00444948", RsidParagraphProperties = "00444948", RsidRunAdditionDefault = "00444948" };
            ...
            Indentation indentation11 = new Indentation() { Start = "-142", End = "-150" };
            ...
            paragraphMarkRunProperties17.Append(fontSizeComplexScript18);

            paragraphProperties17.Append(paragraphBorders9);
            paragraphProperties17.Append(indentation11);
            ...
            paragraph8.Append(run1);

            Paragraph paragraph18 = new Paragraph() { RsidParagraphAddition = "00A65B84", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00A65B84" };
            ...
            Paragraph paragraph19 = new Paragraph() { RsidParagraphAddition = "00444948", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00444948" };
            ...
            Paragraph paragraph20 = new Paragraph() { RsidParagraphAddition = "00444948", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00444948" };
            ...
            Paragraph paragraph21 = new Paragraph() { RsidParagraphMarkRevision = "00D05B95", RsidParagraphAddition = "00444948", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00444948" };
            ...
            Paragraph paragraph22 = new Paragraph() { RsidParagraphMarkRevision = "00D05B95", RsidParagraphAddition = "00A65B84", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00A65B84" };
            ...
            paragraphMarkRunProperties22.Append(bold20);
            ...
            Paragraph paragraph23 = new Paragraph() { RsidParagraphAddition = "00A65B84", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00AA1B96" };
            ...
            paragraphMarkRunProperties23.Append(bold21);
            ...
            Paragraph paragraph24 = new Paragraph() { RsidParagraphAddition = "00AA1B96", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00AA1B96" };
            ...
            paragraphMarkRunProperties24.Append(bold23);
            paragraphMarkRunProperties24.Append(fontSize29);
            ...
            Paragraph paragraph25 = new Paragraph() { RsidParagraphMarkRevision = "00411716", RsidParagraphAddition = "00AA1B96", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00AA1B96" };
            ...
            paragraphMarkRunProperties25.Append(bold25);
            paragraphMarkRunProperties25.Append(fontSize31);
            paragraphMarkRunProperties25.Append(fontSizeComplexScript24);
            ...
            run15.Append(text13);
            BookmarkStart bookmarkStart2 = new BookmarkStart() { Name = "enterName2", Id = "1" };
            BookmarkEnd bookmarkEnd2 = new BookmarkEnd() { Id = "1" };

            Run run16 = new Run() { RsidRunAddition = "00444948" };
            ...
            paragraph25.Append(bookmarkStart2);
            paragraph25.Append(bookmarkEnd2);
            paragraph25.Append(run16);

            Paragraph paragraph26 = new Paragraph() { RsidParagraphMarkRevision = "00411716", RsidParagraphAddition = "00420F1E", RsidParagraphProperties = "00420F1E", RsidRunAdditionDefault = "00420F1E" };
            ...
            paragraphMarkRunProperties26.Append(bold28);
            paragraphMarkRunProperties26.Append(fontSize34);
            paragraphMarkRunProperties26.Append(fontSizeComplexScript27);
            ...
            Paragraph paragraph27 = new Paragraph() { RsidParagraphAddition = "00272583", RsidParagraphProperties = "00444948", RsidRunAdditionDefault = "00420F1E" };
            ...
            paragraphMarkRunProperties27.Append(fontSize35);
            paragraphMarkRunProperties27.Append(fontSizeComplexScript28);

            paragraphProperties27.Append(tabs1);
            paragraphProperties27.Append(indentation12);
            paragraphProperties27.Append(justification27);
            paragraphProperties27.Append(paragraphMarkRunProperties27);

            Run run17 = new Run() { RsidRunProperties = "00411716" };
            ...
            Run run18 = new Run() { RsidRunAddition = "00AA1B96" };
            ...
            runProperties18.Append(fontSize37);
            runProperties18.Append(fontSizeComplexScript30);
            Text text16 = new Text() { Space = SpaceProcessingModeValues.Preserve };
            text16.Text = " разделе 12 личного дела гвардии";

            run18.Append(runProperties18);
            run18.Append(text16);

            Run run19 = new Run() { RsidRunAddition = "00C556A0" };
            ...
            run19.Append(text17);
            BookmarkStart bookmarkStart3 = new BookmarkStart() { Name = "enterName", Id = "2" };
            BookmarkEnd bookmarkEnd3 = new BookmarkEnd() { Id = "2" };

            Run run20 = new Run() { RsidRunAddition = "00444948" };
            ...
            runProperties20.Append(fontSize39);
            ...
            Run run21 = new Run() { RsidRunProperties = "00AA1B96", RsidRunAddition = "00411716" };
            ...
            Run run22 = new Run() { RsidRunAddition = "00AA1B96" };
            ...
            runProperties22.Append(fontSize41);
            runProperties22.Append(fontSizeComplexScript34);
            Text text20 = new Text();
            text20.Text = "значится:";
            ...
            paragraph27.Append(bookmarkStart3);
            paragraph27.Append(bookmarkEnd3);
            paragraph27.Append(run20);
            paragraph27.Append(run21);
            paragraph27.Append(run22);

            Paragraph paragraph28 = new Paragraph() { RsidParagraphAddition = "00272583", RsidParagraphProperties = "00301253", RsidRunAdditionDefault = "00AA1B96" };

            ParagraphProperties paragraphProperties28 = new ParagraphProperties();

            Tabs tabs2 = new Tabs();
            TabStop tabStop2 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };
            ...
            paragraphMarkRunProperties28.Append(fontSizeComplexScript35);

            paragraphProperties28.Append(tabs2);
            ...
            runProperties23.Append(fontSize43);
            runProperties23.Append(fontSizeComplexScript36);
            Text text21 = new Text();
            text21.Text = "Службу проходил:";
            ...
            Paragraph paragraph29 = new Paragraph() { RsidParagraphAddition = "00444948", RsidParagraphProperties = "00301253", RsidRunAdditionDefault = "00444948" };

            ParagraphProperties paragraphProperties29 = new ParagraphProperties();

            Tabs tabs3 = new Tabs();
            TabStop tabStop3 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };
            ...
            FontSize fontSize44 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript37 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties29.Append(fontSize44);
            paragraphMarkRunProperties29.Append(fontSizeComplexScript37);

            paragraphProperties29.Append(tabs3);
            paragraphProperties29.Append(indentation14);
            ...
            runProperties24.Append(fontSize45);
            runProperties24.Append(fontSizeComplexScript38);
            Text text22 = new Text();
            text22.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph30 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00444948" };

            ParagraphProperties paragraphProperties30 = new ParagraphProperties();

            Tabs tabs4 = new Tabs();
            TabStop tabStop4 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs4.Append(tabStop4);
            Indentation indentation15 = new Indentation() { FirstLine = "709" };
            Justification justification30 = new Justification() { Val = JustificationValues.Both };

            ParagraphMarkRunProperties paragraphMarkRunProperties30 = new ParagraphMarkRunProperties();
            FontSize fontSize46 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript39 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties30.Append(fontSize46);
            paragraphMarkRunProperties30.Append(fontSizeComplexScript39);

            paragraphProperties30.Append(tabs4);
            paragraphProperties30.Append(indentation15);
            ...
            runProperties25.Append(fontSize47);
            runProperties25.Append(fontSizeComplexScript40);
            Text text23 = new Text();
            text23.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            paragraph30.Append(run25);
                ...
                {
                    RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764",
                    RsidRunAdditionDefault = "00D55764"
                };

                ParagraphProperties paragraphProperties31 = new ParagraphProperties();

                Tabs tabs5 = new Tabs();
                TabStop tabStop5 = new TabStop() {Val = TabStopValues.Left, Position = 9355};

                tabs5.Append(tabStop5);
                Indentation indentation16 = new Indentation() {FirstLine = "709"};
                Justification justification31 = new Justification() {Val = JustificationValues.Both};

                ParagraphMarkRunProperties paragraphMarkRunProperties31 = new ParagraphMarkRunProperties();
                FontSize fontSize48 = new FontSize() {Val = _historyStrings[_currentHistory] == "" ? "1" : "28"};
                FontSizeComplexScript fontSizeComplexScript41 = new FontSizeComplexScript() {Val = "28"};

                paragraphMarkRunProperties31.Append(fontSize48);
                paragraphMarkRunProperties31.Append(fontSizeComplexScript41);

                paragraphProperties31.Append(tabs5);
                paragraphProperties31.Append(indentation16);
                ...
                runProperties26.Append(fontSize49);
                runProperties26.Append(fontSizeComplexScript42);
                Text text24 = new Text();
                text24.Text = _historyStrings[_currentHistory];
                _currentHistory++;
            ...
            Paragraph paragraph32 = new Paragraph()
                ...
                Tabs tabs6 = new Tabs();
                TabStop tabStop6 = new TabStop() {Val = TabStopValues.Left, Position = 9355};

                tabs6.Append(tabStop6);
                Indentation indentation17 = new Indentation() {FirstLine = "709"};
                Justification justification32 = new Justification() {Val = JustificationValues.Both};

                ParagraphMarkRunProperties paragraphMarkRunProperties32 = new ParagraphMarkRunProperties();
                FontSize fontSize50 = new FontSize() {Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
                FontSizeComplexScript fontSizeComplexScript43 = new FontSizeComplexScript() {Val = "28"};

                paragraphMarkRunProperties32.Append(fontSize50);
                paragraphMarkRunProperties32.Append(fontSizeComplexScript43);

                paragraphProperties32.Append(tabs6);
                paragraphProperties32.Append(indentation17);
            ...
            runProperties27.Append(fontSize51);
            runProperties27.Append(fontSizeComplexScript44);
            Text text25 = new Text();
            text25.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph33 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties33 = new ParagraphProperties();

            Tabs tabs7 = new Tabs();
            TabStop tabStop7 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs7.Append(tabStop7);
            Indentation indentation18 = new Indentation() { FirstLine = "709" };
            Justification justification33 = new Justification() { Val = JustificationValues.Both };

            ParagraphMarkRunProperties paragraphMarkRunProperties33 = new ParagraphMarkRunProperties();
            FontSize fontSize52 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript45 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties33.Append(fontSize52);
            paragraphMarkRunProperties33.Append(fontSizeComplexScript45);

            paragraphProperties33.Append(tabs7);
            paragraphProperties33.Append(indentation18);
            ...
            runProperties28.Append(fontSize53);
            runProperties28.Append(fontSizeComplexScript46);
            Text text26 = new Text();
            text26.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            paragraph33.Append(run28);

            Paragraph paragraph34 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties34 = new ParagraphProperties();

            Tabs tabs8 = new Tabs();
            TabStop tabStop8 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs8.Append(tabStop8);
            Indentation indentation19 = new Indentation() { FirstLine = "709" };
            Justification justification34 = new Justification() { Val = JustificationValues.Both };

            ParagraphMarkRunProperties paragraphMarkRunProperties34 = new ParagraphMarkRunProperties();
            FontSize fontSize54 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript47 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties34.Append(fontSize54);
            paragraphMarkRunProperties34.Append(fontSizeComplexScript47);

            paragraphProperties34.Append(tabs8);
            paragraphProperties34.Append(indentation19);
            ...
            runProperties29.Append(fontSize55);
            runProperties29.Append(fontSizeComplexScript48);
            Text text27 = new Text();
            text27.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            paragraph34.Append(run29);

            Paragraph paragraph35 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties35 = new ParagraphProperties();

            Tabs tabs9 = new Tabs();
            TabStop tabStop9 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs9.Append(tabStop9);
            Indentation indentation20 = new Indentation() { FirstLine = "709" };
            Justification justification35 = new Justification() { Val = JustificationValues.Both };

            ParagraphMarkRunProperties paragraphMarkRunProperties35 = new ParagraphMarkRunProperties();
            FontSize fontSize56 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript49 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties35.Append(fontSize56);
            paragraphMarkRunProperties35.Append(fontSizeComplexScript49);

            paragraphProperties35.Append(tabs9);
            paragraphProperties35.Append(indentation20);
            ...
            runProperties30.Append(fontSize57);
            runProperties30.Append(fontSizeComplexScript50);
            Text text28 = new Text();
            text28.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph36 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties36 = new ParagraphProperties();

            Tabs tabs10 = new Tabs();
            TabStop tabStop10 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs10.Append(tabStop10);
            Indentation indentation21 = new Indentation() { FirstLine = "709" };
            Justification justification36 = new Justification() { Val = JustificationValues.Both };

            ParagraphMarkRunProperties paragraphMarkRunProperties36 = new ParagraphMarkRunProperties();
            FontSize fontSize58 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            ...
            paragraphProperties36.Append(tabs10);
            paragraphProperties36.Append(indentation21);
            ...
            runProperties31.Append(fontSize59);
            runProperties31.Append(fontSizeComplexScript52);
            Text text29 = new Text();
            text29.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph37 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties37 = new ParagraphProperties();

            Tabs tabs11 = new Tabs();
            TabStop tabStop11 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs11.Append(tabStop11);
            Indentation indentation22 = new Indentation() { FirstLine = "709" };
            Justification justification37 = new Justification() { Val = JustificationValues.Both };

            ParagraphMarkRunProperties paragraphMarkRunProperties37 = new ParagraphMarkRunProperties();
            FontSize fontSize60 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            ...
            paragraphMarkRunProperties37.Append(fontSizeComplexScript53);

            paragraphProperties37.Append(tabs11);
            paragraphProperties37.Append(indentation22);
            ...
            runProperties32.Append(fontSizeComplexScript54);
            Text text30 = new Text();
            text30.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph38 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties38 = new ParagraphProperties();

            Tabs tabs12 = new Tabs();
            TabStop tabStop12 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs12.Append(tabStop12);
            Indentation indentation23 = new Indentation() { FirstLine = "709" };
            Justification justification38 = new Justification() { Val = JustificationValues.Both };

            ParagraphMarkRunProperties paragraphMarkRunProperties38 = new ParagraphMarkRunProperties();
            FontSize fontSize62 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            ...
            paragraphMarkRunProperties38.Append(fontSizeComplexScript55);

            paragraphProperties38.Append(tabs12);
            paragraphProperties38.Append(indentation23);
            ...
            runProperties33.Append(fontSizeComplexScript56);
            Text text31 = new Text();
            text31.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph39 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties39 = new ParagraphProperties();

            Tabs tabs13 = new Tabs();
            TabStop tabStop13 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs13.Append(tabStop13);
            Indentation indentation24 = new Indentation() { FirstLine = "709" };
            Justification justification39 = new Justification() { Val = JustificationValues.Both };

            ParagraphMarkRunProperties paragraphMarkRunProperties39 = new ParagraphMarkRunProperties();
            FontSize fontSize64 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            ...
            paragraphMarkRunProperties39.Append(fontSizeComplexScript57);

            paragraphProperties39.Append(tabs13);
            paragraphProperties39.Append(indentation24);
            ...
            runProperties34.Append(fontSize65);
            runProperties34.Append(fontSizeComplexScript58);
            Text text32 = new Text();
            text32.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph40 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties40 = new ParagraphProperties();

            Tabs tabs14 = new Tabs();
            TabStop tabStop14 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };
            ...
            FontSize fontSize66 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript59 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties40.Append(fontSize66);
            paragraphMarkRunProperties40.Append(fontSizeComplexScript59);

            paragraphProperties40.Append(tabs14);
            paragraphProperties40.Append(indentation25);
            ...
            runProperties35.Append(fontSizeComplexScript60);
            Text text33 = new Text();
            text33.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph41 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties41 = new ParagraphProperties();

            Tabs tabs15 = new Tabs();
            TabStop tabStop15 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs15.Append(tabStop15);
            Indentation indentation26 = new Indentation() { FirstLine = "709" };
            Justification justification41 = new Justification() { Val = JustificationValues.Both };

            ParagraphMarkRunProperties paragraphMarkRunProperties41 = new ParagraphMarkRunProperties();
            FontSize fontSize68 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            ...
            paragraphMarkRunProperties41.Append(fontSizeComplexScript61);

            paragraphProperties41.Append(tabs15);
            paragraphProperties41.Append(indentation26);
            ...
            runProperties36.Append(fontSize69);
            runProperties36.Append(fontSizeComplexScript62);
            Text text34 = new Text();
            text34.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph42 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties42 = new ParagraphProperties();

            Tabs tabs16 = new Tabs();
            TabStop tabStop16 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs16.Append(tabStop16);
            Indentation indentation27 = new Indentation() { FirstLine = "709" };
            Justification justification42 = new Justification() { Val = JustificationValues.Both };

            ParagraphMarkRunProperties paragraphMarkRunProperties42 = new ParagraphMarkRunProperties();
            FontSize fontSize70 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript63 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties42.Append(fontSize70);
            paragraphMarkRunProperties42.Append(fontSizeComplexScript63);

            paragraphProperties42.Append(tabs16);
            paragraphProperties42.Append(indentation27);
            ...
            runProperties37.Append(fontSizeComplexScript64);
            Text text35 = new Text();
            text35.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph43 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties43 = new ParagraphProperties();

            Tabs tabs17 = new Tabs();
            TabStop tabStop17 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs17.Append(tabStop17);
            Indentation indentation28 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize72 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            ...
            paragraphMarkRunProperties43.Append(fontSizeComplexScript65);

            paragraphProperties43.Append(tabs17);
            paragraphProperties43.Append(indentation28);
            ...
            runProperties38.Append(fontSize73);
            runProperties38.Append(fontSizeComplexScript66);
            Text text36 = new Text();
            text36.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph44 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties44 = new ParagraphProperties();

            Tabs tabs18 = new Tabs();
            TabStop tabStop18 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs18.Append(tabStop18);
            Indentation indentation29 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize74 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript67 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties44.Append(fontSize74);
            paragraphMarkRunProperties44.Append(fontSizeComplexScript67);

            paragraphProperties44.Append(tabs18);
            paragraphProperties44.Append(indentation29);
            ...
            runProperties39.Append(fontSize75);
            runProperties39.Append(fontSizeComplexScript68);
            Text text37 = new Text();
            text37.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            paragraph44.Append(run39);

            Paragraph paragraph45 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties45 = new ParagraphProperties();

            Tabs tabs19 = new Tabs();
            TabStop tabStop19 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs19.Append(tabStop19);
            Indentation indentation30 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize76 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript69 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties45.Append(fontSize76);
            paragraphMarkRunProperties45.Append(fontSizeComplexScript69);

            paragraphProperties45.Append(tabs19);
            paragraphProperties45.Append(indentation30);
            ...
            runProperties40.Append(fontSize77);
            runProperties40.Append(fontSizeComplexScript70);
            Text text38 = new Text();
            text38.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            paragraph45.Append(run40);

            Paragraph paragraph46 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties46 = new ParagraphProperties();

            Tabs tabs20 = new Tabs();
            TabStop tabStop20 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs20.Append(tabStop20);
            Indentation indentation31 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize78 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript71 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties46.Append(fontSize78);
            paragraphMarkRunProperties46.Append(fontSizeComplexScript71);

            paragraphProperties46.Append(tabs20);
            paragraphProperties46.Append(indentation31);
            ...
            runProperties41.Append(fontSize79);
            runProperties41.Append(fontSizeComplexScript72);
            Text text39 = new Text();
            text39.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            paragraph46.Append(run41);

            Paragraph paragraph47 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties47 = new ParagraphProperties();

            Tabs tabs21 = new Tabs();
            TabStop tabStop21 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs21.Append(tabStop21);
            Indentation indentation32 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize80 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript73 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties47.Append(fontSize80);
            paragraphMarkRunProperties47.Append(fontSizeComplexScript73);

            paragraphProperties47.Append(tabs21);
            paragraphProperties47.Append(indentation32);
            ...
            runProperties42.Append(fontSize81);
            runProperties42.Append(fontSizeComplexScript74);
            Text text40 = new Text();
            text40.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            paragraph47.Append(run42);

            Paragraph paragraph48 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties48 = new ParagraphProperties();

            Tabs tabs22 = new Tabs();
            TabStop tabStop22 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs22.Append(tabStop22);
            Indentation indentation33 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize82 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript75 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties48.Append(fontSize82);
            paragraphMarkRunProperties48.Append(fontSizeComplexScript75);

            paragraphProperties48.Append(tabs22);
            paragraphProperties48.Append(indentation33);
            paragraphProperties48.Append(justification48);
            ...
            runProperties43.Append(fontSize83);
            runProperties43.Append(fontSizeComplexScript76);
            Text text41 = new Text();
            text41.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            paragraph48.Append(run43);

            Paragraph paragraph49 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties49 = new ParagraphProperties();

            Tabs tabs23 = new Tabs();
            TabStop tabStop23 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs23.Append(tabStop23);
            Indentation indentation34 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize84 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript77 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties49.Append(fontSize84);
            paragraphMarkRunProperties49.Append(fontSizeComplexScript77);

            paragraphProperties49.Append(tabs23);
            paragraphProperties49.Append(indentation34);
            paragraphProperties49.Append(justification49);
            ...
            runProperties44.Append(fontSize85);
            runProperties44.Append(fontSizeComplexScript78);
            Text text42 = new Text();
            text42.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            paragraph49.Append(run44);

            Paragraph paragraph50 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties50 = new ParagraphProperties();

            Tabs tabs24 = new Tabs();
            TabStop tabStop24 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs24.Append(tabStop24);
            Indentation indentation35 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize86 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript79 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties50.Append(fontSize86);
            paragraphMarkRunProperties50.Append(fontSizeComplexScript79);

            paragraphProperties50.Append(tabs24);
            paragraphProperties50.Append(indentation35);
            paragraphProperties50.Append(justification50);
            ...
            runProperties45.Append(fontSize87);
            runProperties45.Append(fontSizeComplexScript80);
            Text text43 = new Text();
            text43.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph51 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties51 = new ParagraphProperties();

            Tabs tabs25 = new Tabs();
            TabStop tabStop25 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs25.Append(tabStop25);
            Indentation indentation36 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize88 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript81 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties51.Append(fontSize88);
            paragraphMarkRunProperties51.Append(fontSizeComplexScript81);

            paragraphProperties51.Append(tabs25);
            paragraphProperties51.Append(indentation36);
            paragraphProperties51.Append(justification51);
            ...
            runProperties46.Append(fontSize89);
            runProperties46.Append(fontSizeComplexScript82);
            Text text44 = new Text();
            text44.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph52 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties52 = new ParagraphProperties();

            Tabs tabs26 = new Tabs();
            TabStop tabStop26 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs26.Append(tabStop26);
            Indentation indentation37 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize90 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript83 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties52.Append(fontSize90);
            paragraphMarkRunProperties52.Append(fontSizeComplexScript83);

            paragraphProperties52.Append(tabs26);
            paragraphProperties52.Append(indentation37);
            paragraphProperties52.Append(justification52);
            ...
            runProperties47.Append(fontSize91);
            runProperties47.Append(fontSizeComplexScript84);
            Text text45 = new Text();
            text45.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            paragraph52.Append(run47);

            Paragraph paragraph53 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties53 = new ParagraphProperties();

            Tabs tabs27 = new Tabs();
            TabStop tabStop27 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs27.Append(tabStop27);
            Indentation indentation38 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize92 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript85 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties53.Append(fontSize92);
            paragraphMarkRunProperties53.Append(fontSizeComplexScript85);

            paragraphProperties53.Append(tabs27);
            paragraphProperties53.Append(indentation38);
            paragraphProperties53.Append(justification53);
            ...
            runProperties48.Append(fontSize93);
            runProperties48.Append(fontSizeComplexScript86);
            Text text46 = new Text();
            text46.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            paragraph53.Append(run48);

            Paragraph paragraph54 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties54 = new ParagraphProperties();

            Tabs tabs28 = new Tabs();
            TabStop tabStop28 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs28.Append(tabStop28);
            Indentation indentation39 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize94 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript87 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties54.Append(fontSize94);
            paragraphMarkRunProperties54.Append(fontSizeComplexScript87);

            paragraphProperties54.Append(tabs28);
            paragraphProperties54.Append(indentation39);
            paragraphProperties54.Append(justification54);
            ...
            runProperties49.Append(fontSize95);
            runProperties49.Append(fontSizeComplexScript88);
            Text text47 = new Text();
            text47.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph55 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties55 = new ParagraphProperties();

            Tabs tabs29 = new Tabs();
            TabStop tabStop29 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs29.Append(tabStop29);
            Indentation indentation40 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize96 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            ...
            paragraphMarkRunProperties55.Append(fontSizeComplexScript89);

            paragraphProperties55.Append(tabs29);
            paragraphProperties55.Append(indentation40);
            paragraphProperties55.Append(justification55);
            ...
            runProperties50.Append(fontSize97);
            runProperties50.Append(fontSizeComplexScript90);
            Text text48 = new Text();
            text48.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph56 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties56 = new ParagraphProperties();

            Tabs tabs30 = new Tabs();
            TabStop tabStop30 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs30.Append(tabStop30);
            Indentation indentation41 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize98 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            ...
            paragraphMarkRunProperties56.Append(fontSizeComplexScript91);

            paragraphProperties56.Append(tabs30);
            paragraphProperties56.Append(indentation41);
            paragraphProperties56.Append(justification56);
            ...
            runProperties51.Append(fontSize99);
            runProperties51.Append(fontSizeComplexScript92);
            Text text49 = new Text();
            text49.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph57 = new Paragraph() { RsidParagraphAddition = "00D55764", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties57 = new ParagraphProperties();

            Tabs tabs31 = new Tabs();
            TabStop tabStop31 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs31.Append(tabStop31);
            Indentation indentation42 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize100 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript93 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties57.Append(fontSize100);
            paragraphMarkRunProperties57.Append(fontSizeComplexScript93);

            paragraphProperties57.Append(tabs31);
            paragraphProperties57.Append(indentation42);
            paragraphProperties57.Append(justification57);
            ...
            runProperties52.Append(fontSizeComplexScript94);
            LastRenderedPageBreak lastRenderedPageBreak1 = new LastRenderedPageBreak();
            Text text50 = new Text();
            text50.Text = _historyStrings[_currentHistory];
            _currentHistory++;

            run52.Append(runProperties52);
            run52.Append(lastRenderedPageBreak1);
            run52.Append(text50);

            paragraph57.Append(paragraphProperties57);
            paragraph57.Append(run52);

            Paragraph paragraph58 = new Paragraph() { RsidParagraphAddition = "00444948", RsidParagraphProperties = "00D55764", RsidRunAdditionDefault = "00D55764" };

            ParagraphProperties paragraphProperties58 = new ParagraphProperties();

            Tabs tabs32 = new Tabs();
            TabStop tabStop32 = new TabStop() { Val = TabStopValues.Left, Position = 9355 };

            tabs32.Append(tabStop32);
            Indentation indentation43 = new Indentation() { FirstLine = "709" };
            ...
            FontSize fontSize102 = new FontSize() { Val = _historyStrings[_currentHistory] == "" ? "1" : "28" };
            FontSizeComplexScript fontSizeComplexScript95 = new FontSizeComplexScript() { Val = "28" };

            paragraphMarkRunProperties58.Append(fontSize102);
            paragraphMarkRunProperties58.Append(fontSizeComplexScript95);

            paragraphProperties58.Append(tabs32);
            paragraphProperties58.Append(indentation43);
            paragraphProperties58.Append(justification58);
            ...
            runProperties53.Append(fontSize103);
            runProperties53.Append(fontSizeComplexScript96);
            Text text51 = new Text();
            text51.Text = _historyStrings[_currentHistory];
            _currentHistory++;
            ...
            Paragraph paragraph59 = new Paragraph() { RsidParagraphMarkRevision = "00411716", RsidParagraphAddition = "00420F1E", RsidParagraphProperties = "00411716", RsidRunAdditionDefault = "00AA1B96" };

            ParagraphProperties paragraphProperties59 = new ParagraphProperties();
            Indentation indentation44 = new Indentation() { FirstLine = "709" };
            ...
            paragraphMarkRunProperties59.Append(fontSize104);
            paragraphMarkRunProperties59.Append(fontSizeComplexScript97);

            paragraphProperties59.Append(indentation44);
            paragraphProperties59.Append(justification59);
            paragraphProperties59.Append(paragraphMarkRunProperties59);
            BookmarkStart bookmarkStart4 = new BookmarkStart() { Name = "positions1", Id = "3" };
            ...
            runProperties54.Append(fontSize105);
            runProperties54.Append(fontSizeComplexScript98);
            Text text52 = new Text();
            text52.Text = "Контракт о прох";

            run54.Append(runProperties54);
            run54.Append(text52);

            Run run55 = new Run() { RsidRunAddition = "00027C3F" };
            ...
            runProperties55.Append(fontSize106);
            runProperties55.Append(fontSizeComplexScript99);
            Text text53 = new Text();
            text53.Text = "ождении военной службы заключен";

            run55.Append(runProperties55);
            run55.Append(text53);

            Run run56 = new Run() { RsidRunAddition = "00A65B84" };
            ...
            runProperties56.Append(fontSize107);
            runProperties56.Append(fontSizeComplexScript100);
            Text text54 = new Text() { Space = SpaceProcessingModeValues.Preserve };
            text54.Text = " ";

            run56.Append(runProperties56);
            run56.Append(text54);
            BookmarkStart bookmarkStart5 = new BookmarkStart() { Name = "enterKSLength", Id = "4" };
            BookmarkEnd bookmarkEnd5 = new BookmarkEnd() { Id = "4" };

            Run run57 = new Run() { RsidRunAddition = "00444948" };
            ...
            runProperties57.Append(fontSizeComplexScript101);
            Text text55 = new Text();
            text55.Text = _slaveString;
            ...
            Run run59 = new Run() { RsidRunAddition = "00444948" };
            ...
            runProperties59.Append(fontSize110);
            runProperties59.Append(fontSizeComplexScript103);
            ...
            Run run60 = new Run() { RsidRunAddition = "00C47A14" };
            ...
            runProperties60.Append(fontSize111);
            runProperties60.Append(fontSizeComplexScript104);
            ...
            Run run61 = new Run() { RsidRunAddition = "008C180A" };
            ...
            runProperties61.Append(fontSize112);
            runProperties61.Append(fontSizeComplexScript105);
            ...
            paragraph59.Append(run60);
            paragraph59.Append(run61);

            Paragraph paragraph60 = new Paragraph() { RsidParagraphAddition = "00420F1E", RsidParagraphProperties = "00420F1E", RsidRunAdditionDefault = "00420F1E" };
            ...
            paragraphMarkRunProperties60.Append(fontSizeComplexScript106);

            paragraphProperties60.Append(justification60);
            paragraphProperties60.Append(paragraphMarkRunProperties60);

            paragraph60.Append(paragraphProperties60);

            Paragraph paragraph61 = new Paragraph() { RsidParagraphMarkRevision = "008F540A", RsidParagraphAddition = "00F5360A", RsidParagraphProperties = "00420F1E", RsidRunAdditionDefault = "00F5360A" };
            ...
            paragraphMarkRunProperties61.Append(bold29);
            paragraphMarkRunProperties61.Append(fontSize114);
            paragraphMarkRunProperties61.Append(fontSizeComplexScript107);

            paragraphProperties61.Append(justification61);
            paragraphProperties61.Append(paragraphMarkRunProperties61);

            paragraph61.Append(paragraphProperties61);

            Paragraph paragraph62 = new Paragraph() { RsidParagraphMarkRevision = "008F540A", RsidParagraphAddition = "00420F1E", RsidParagraphProperties = "00A65B84", RsidRunAdditionDefault = "00AA1B96" };

            ParagraphProperties paragraphProperties62 = new ParagraphProperties();

            Tabs tabs33 = new Tabs();
            TabStop tabStop33 = new TabStop() { Val = TabStopValues.Left, Position = 6480 };
            ...
            paragraphMarkRunProperties62.Append(bold30);
            paragraphMarkRunProperties62.Append(fontSize115);
            paragraphMarkRunProperties62.Append(fontSizeComplexScript108);

            paragraphProperties62.Append(tabs33);
            ...
            runProperties62.Append(bold31);
            runProperties62.Append(fontSize116);
            runProperties62.Append(fontSizeComplexScript109);
            ...
            Run run63 = new Run() { RsidRunAddition = "00444948" };
            ...
            runProperties63.Append(bold32);
            runProperties63.Append(fontSize117);
            runProperties63.Append(fontSizeComplexScript110);
            Text text61 = new Text();
            text61.Text = !_nok[1] ? "Начальник отделения кадров" : "Врио начальника отделения кадров";

            run63.Append(runProperties63);
            run63.Append(text61);

            Run run64 = new Run() { RsidRunProperties = "008F540A", RsidRunAddition = "007D79D6" };
            ...
            runProperties64.Append(bold33);
            runProperties64.Append(fontSize118);
            runProperties64.Append(fontSizeComplexScript111);
            ...
            Run run65 = new Run() { RsidRunProperties = "008F540A", RsidRunAddition = "00A65B84" };
            ...
            runProperties65.Append(bold34);
            runProperties65.Append(fontSize119);
            runProperties65.Append(fontSizeComplexScript112);
            Text text63 = new Text() { Space = SpaceProcessingModeValues.Preserve };
            text63.Text = "войсковой части ";

            run65.Append(runProperties65);
            run65.Append(text63);

            Run run66 = new Run() { RsidRunProperties = "008F540A", RsidRunAddition = "00DE6D70" };
            ...
            runProperties66.Append(bold35);
            runProperties66.Append(fontSize120);
            ...
            paragraph62.Append(bookmarkStart7);
            paragraph62.Append(bookmarkEnd7);
            paragraph62.Append(run63);
            paragraph62.Append(run64);
            paragraph62.Append(run65);
            paragraph62.Append(run66);

            Paragraph paragraph63 = new Paragraph() { RsidParagraphMarkRevision = "008F540A", RsidParagraphAddition = "00411716", RsidParagraphProperties = "007D79D6", RsidRunAdditionDefault = "00061E63" };

            ParagraphProperties paragraphProperties63 = new ParagraphProperties();

            Tabs tabs34 = new Tabs();
            TabStop tabStop34 = new TabStop() { Val = TabStopValues.Left, Position = 3402 };
            ...
            paragraphMarkRunProperties63.Append(bold36);
            paragraphMarkRunProperties63.Append(fontSize121);
            paragraphMarkRunProperties63.Append(fontSizeComplexScript114);

            paragraphProperties63.Append(tabs34);
            paragraphProperties63.Append(paragraphMarkRunProperties63);

            Run run67 = new Run() { RsidRunProperties = "008F540A" };
            ...
            runProperties67.Append(bold37);
            runProperties67.Append(fontSize122);
            ...
            Run run68 = new Run() { RsidRunProperties = "008F540A", RsidRunAddition = "007D79D6" };
            ...
            runProperties68.Append(bold38);
            runProperties68.Append(fontSize123);
            runProperties68.Append(fontSizeComplexScript116);
            ...
            Run run69 = new Run() { RsidRunAddition = "00444948" };
            ...
            runProperties69.Append(bold39);
            runProperties69.Append(fontSize124);
            runProperties69.Append(fontSizeComplexScript117);
            Text text67 = new Text();
            text67.Text = _nokPrimary;

            run69.Append(runProperties69);
            run69.Append(text67);

            paragraph63.Append(paragraphProperties63);
            paragraph63.Append(run67);
            paragraph63.Append(run68);
            paragraph63.Append(bookmarkStart8);
            paragraph63.Append(bookmarkEnd8);
            paragraph63.Append(run69);

            Paragraph paragraph64 = new Paragraph() { RsidParagraphAddition = "00C33205", RsidParagraphProperties = "00C33205", RsidRunAdditionDefault = "00444948" };

            ParagraphProperties paragraphProperties64 = new ParagraphProperties();
            Justification justification62 = new Justification() { Val = JustificationValues.Right };
            ...
            paragraphMarkRunProperties64.Append(bold40);
            paragraphMarkRunProperties64.Append(fontSize125);

            paragraphProperties64.Append(justification62);
            ...
            runProperties70.Append(bold41);
            runProperties70.Append(fontSize126);
            ...
            paragraph64.Append(bookmarkStart9);
            paragraph64.Append(bookmarkEnd9);
            paragraph64.Append(run70);

            Paragraph paragraph65 = new Paragraph() { RsidParagraphAddition = "00C33205", RsidParagraphProperties = "00C33205", RsidRunAdditionDefault = "00C33205" };
            ...
            paragraphMarkRunProperties65.Append(fontSize127);
            ...
            runProperties71.Append(fontSize128);
            ...
            paragraph65.Append(run71);

            SectionProperties sectionProperties1 = new SectionProperties() { RsidR = "00C33205", RsidSect = "00A65B84" };
        ...
        private void GenerateWebSettingsPart1Content(WebSettingsPart webSettingsPart1)
        {
            ...
            Div div2 = new Div() { Id = "1780180988" };
            ...
            divBorder2.Append(topBorder11);
            divBorder2.Append(leftBorder11);
            divBorder2.Append(bottomBorder11);
            divBorder2.Append(rightBorder11);
        ...
        private void GenerateDocumentSettingsPart1Content(DocumentSettingsPart documentSettingsPart1)
        {
            ...
            Rsid rsid1 = new Rsid() { Val = "00013587" };
            Rsid rsid2 = new Rsid() { Val = "00027C3F" };
            Rsid rsid3 = new Rsid() { Val = "00061E63" };
            Rsid rsid4 = new Rsid() { Val = "00072488" };
            Rsid rsid5 = new Rsid() { Val = "00075349" };
            Rsid rsid6 = new Rsid() { Val = "000F198A" };
            Rsid rsid7 = new Rsid() { Val = "000F4EB6" };
            Rsid rsid8 = new Rsid() { Val = "001269A7" };
            Rsid rsid9 = new Rsid() { Val = "00133382" };
            Rsid rsid10 = new Rsid() { Val = "00136F39" };
            Rsid rsid11 = new Rsid() { Val = "00160560" };
            Rsid rsid12 = new Rsid() { Val = "002333ED" };
            Rsid rsid13 = new Rsid() { Val = "0024309E" };
            Rsid rsid14 = new Rsid() { Val = "00262ED6" };
            Rsid rsid15 = new Rsid() { Val = "00272583" };
            Rsid rsid16 = new Rsid() { Val = "0029644B" };
            Rsid rsid17 = new Rsid() { Val = "00301253" };
            Rsid rsid18 = new Rsid() { Val = "00302436" };
            Rsid rsid19 = new Rsid() { Val = "004046FF" };
            Rsid rsid20 = new Rsid() { Val = "00411716" };
            Rsid rsid21 = new Rsid() { Val = "00414B7B" };
            Rsid rsid22 = new Rsid() { Val = "00420F1E" };
            Rsid rsid23 = new Rsid() { Val = "00422744" };
            Rsid rsid24 = new Rsid() { Val = "00444948" };
            Rsid rsid25 = new Rsid() { Val = "004734D9" };
            Rsid rsid26 = new Rsid() { Val = "004846CC" };
            Rsid rsid27 = new Rsid() { Val = "004A4AF7" };
            Rsid rsid28 = new Rsid() { Val = "004A5031" };
            Rsid rsid29 = new Rsid() { Val = "004D1219" };
            Rsid rsid30 = new Rsid() { Val = "005C7B0E" };
            Rsid rsid31 = new Rsid() { Val = "006357F7" };
            Rsid rsid32 = new Rsid() { Val = "00655D1D" };
            Rsid rsid33 = new Rsid() { Val = "0067553C" };
            Rsid rsid34 = new Rsid() { Val = "006A29CC" };
            Rsid rsid35 = new Rsid() { Val = "006E4693" };
            Rsid rsid36 = new Rsid() { Val = "006F0BF7" };
            Rsid rsid37 = new Rsid() { Val = "006F20FB" };
            Rsid rsid38 = new Rsid() { Val = "007035E7" };
            Rsid rsid39 = new Rsid() { Val = "00743855" };
            Rsid rsid40 = new Rsid() { Val = "007D79D6" };
            Rsid rsid41 = new Rsid() { Val = "00843534" };
            Rsid rsid42 = new Rsid() { Val = "008B2FAA" };
            Rsid rsid43 = new Rsid() { Val = "008C180A" };
            Rsid rsid44 = new Rsid() { Val = "008E3157" };
            Rsid rsid45 = new Rsid() { Val = "008F540A" };
            Rsid rsid46 = new Rsid() { Val = "009B1818" };
            Rsid rsid47 = new Rsid() { Val = "00A5672D" };
            Rsid rsid48 = new Rsid() { Val = "00A5720A" };
            Rsid rsid49 = new Rsid() { Val = "00A65B84" };
            Rsid rsid50 = new Rsid() { Val = "00A86B46" };
            Rsid rsid51 = new Rsid() { Val = "00A9411A" };
            Rsid rsid52 = new Rsid() { Val = "00AA1B96" };
            Rsid rsid53 = new Rsid() { Val = "00B0030A" };
            Rsid rsid54 = new Rsid() { Val = "00B028E4" };
            Rsid rsid55 = new Rsid() { Val = "00C065C6" };
            Rsid rsid56 = new Rsid() { Val = "00C31C88" };
            Rsid rsid57 = new Rsid() { Val = "00C33205" };
            Rsid rsid58 = new Rsid() { Val = "00C40E40" };
            Rsid rsid59 = new Rsid() { Val = "00C47A14" };
            Rsid rsid60 = new Rsid() { Val = "00C556A0" };
            Rsid rsid61 = new Rsid() { Val = "00C9078A" };
            Rsid rsid62 = new Rsid() { Val = "00CB447F" };
            Rsid rsid63 = new Rsid() { Val = "00CD4D62" };
            Rsid rsid64 = new Rsid() { Val = "00CD6AC5" };
            Rsid rsid65 = new Rsid() { Val = "00CF47AF" };
            Rsid rsid66 = new Rsid() { Val = "00D55764" };
            Rsid rsid67 = new Rsid() { Val = "00DD1F1F" };
            Rsid rsid68 = new Rsid() { Val = "00DE4E76" };
            Rsid rsid69 = new Rsid() { Val = "00DE6D70" };
            Rsid rsid70 = new Rsid() { Val = "00E20DA9" };
            Rsid rsid71 = new Rsid() { Val = "00F5360A" };
            Rsid rsid72 = new Rsid() { Val = "00F813C2" };
            Rsid rsid73 = new Rsid() { Val = "00F8398B" };
            Rsid rsid74 = new Rsid() { Val = "00FE0CE4" };
            Rsid rsid75 = new Rsid() { Val = "00FF7D83" };
            ...
            W15.PersistentDocumentId persistentDocumentId1 = new W15.PersistentDocumentId() { Val = "{D9F6F1FC-9022-4051-9E91-28448D3475B2}" };
        ...
        private void GenerateStyleDefinitionsPart1Content(StyleDefinitionsPart styleDefinitionsPart1)
        {
            ...
            runPropertiesBaseStyle1.Append(fontSize129);
            runPropertiesBaseStyle1.Append(fontSizeComplexScript118);
            ...
            Rsid rsid76 = new Rsid() { Val = "00420F1E" };
            ...
            styleRunProperties1.Append(fontSize130);
            styleRunProperties1.Append(fontSizeComplexScript119);
            styleRunProperties1.Append(languages2);

            style1.Append(styleName1);
            style1.Append(primaryStyle1);
            style1.Append(rsid76);
            ...
            Rsid rsid77 = new Rsid() { Val = "004846CC" };
            ...
            TopBorder topBorder12 = new TopBorder() { Val = BorderValues.Single, Color = "000000", ThemeColor = ThemeColorValues.Text1, Size = (UInt32Value)4U, Space = (UInt32Value)0U };
            LeftBorder leftBorder12 = new LeftBorder() { Val = BorderValues.Single, Color = "000000", ThemeColor = ThemeColorValues.Text1, Size = (UInt32Value)4U, Space = (UInt32Value)0U };
            BottomBorder bottomBorder12 = new BottomBorder() { Val = BorderValues.Single, Color = "000000", ThemeColor = ThemeColorValues.Text1, Size = (UInt32Value)4U, Space = (UInt32Value)0U };
            RightBorder rightBorder12 = new RightBorder() { Val = BorderValues.Single, Color = "000000", ThemeColor = ThemeColorValues.Text1, Size = (UInt32Value)4U, Space = (UInt32Value)0U };
            ...
            tableBorders1.Append(topBorder12);
            tableBorders1.Append(leftBorder12);
            tableBorders1.Append(bottomBorder12);
            tableBorders1.Append(rightBorder12);
        ...
        private void GenerateFontTablePart1Content(FontTablePart fontTablePart1)
        {
            ...
            Font font4 = new Font() { Name = "Arial Narrow" };
            Panose1Number panose1Number4 = new Panose1Number() { Val = "020B0606020202030204" };
            ...
            FontSignature fontSignature4 = new FontSignature() { UnicodeSignature0 = "00000287", UnicodeSignature1 = "00000800", UnicodeSignature2 = "00000000", UnicodeSignature3 = "00000000", CodePageSignature0 = "0000009F", CodePageSignature1 = "00000000" };
        ...
        private void SetPackageProperties(OpenXmlPackage document)
        {
            ...
            document.PackageProperties.Revision = "55";
            ...
            document.PackageProperties.Modified = System.Xml.XmlConvert.ToDateTime("2019-08-12T02:19:00Z", System.Xml.XmlDateTimeSerializationMode.RoundtripKind);
